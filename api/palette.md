...
can specify a `numberOfColors` (an integer value).

`otherPalette` can be another palette object to create a copy of it.

If `fromFile` is given, the palette is loaded from the given filename.

If `fromResource` is given, the `resourceID` is an ID specified in one
of the [extension
palettes](https://github.com/aseprite/aseprite/tree/main/data/extensions)
(e.g.
...
Changes the number of palette colors to `ncolors` (an integer
value).
